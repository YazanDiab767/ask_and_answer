$(document).ready(function () {
            ...
            success: function (data) {
                ...
                $("#colleges").prepend(`
                    <tr class="college">
                        ...
                        <td class="align-middle count_majors"> 0 </td>
                        ...
                        <td class="align-middle text-center" style="width: 12%;">
                        <a href="${data.id}/${data.name}" class="btn btn-success btnEditCollege w-100" data-toggle="modal" data-target=".editCollege"> <i class="fas fa-edit"></i> Edit  </a>
                        </td>
                        <td class="align-middle text-center" style="width: 12%;">
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                thisRow.closest('.college').html(`
                        ...
                        <td class="align-middle"> <b> Modified now </b> </td>
                        <td class="align-middle"> ${data.name} </td>
                        <td class="align-middle"> <img class="rounded" width="150" height="100" src="${host}storage/${data.image}" /> </td>

                        <td class="align-middle count_majors"> ${data.majors.length} </td>
    ...
    $('body').on('click', '.btnShowMajors', function (e) {
        ...
        count_majors = $(this).closest('.college').find('.count_majors');

        college_name = data.split('/')[1];
        college_id = data.split('/')[0];

        $(".countMajors").html(`${college_name} ( ${count_majors.html()} )`);

        let token = $("meta[name='csrf-token']").attr("content");

        //get majors for college
        let url = `${host}college/getMajors/${college_id}`
    ...
    $("#setMajor").on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                count_majors.html(parseInt(count_majors.html()) + 1);
                $(".countMajors").html(`( ${count_majors.html()} )`);
    ...
    // filter function
    (function ($) {
        $("#majors tbody").addClass("search");
        $('#searchInputMajor').keyup(function () {
            var rex = new RegExp($(this).val(), 'i');
            $('.table_majors > .search tr').hide();
            //Recusively filter the jquery object to get results.
            $('.table_majors > .search tr').filter(function (i, v) {
