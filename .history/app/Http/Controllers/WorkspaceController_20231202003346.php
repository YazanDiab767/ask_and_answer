...
class WorkspaceController extends Controller
{
    ...
    {
        $tasks = json_decode($workspace->tasks);
        $user_tasks = '';

        if ( property_exists($tasks, $request->username) ) // the user has already tasks, so add new task to all
        {
            $user_tasks = $tasks[ $request->username ];
        }


        $user_tasks[ date("Y-m-d H:i:s") ] = array(
            "title" => $request->title,
            "date" => $request->date,
            "isDone" => false,
            "addBy" => ""
